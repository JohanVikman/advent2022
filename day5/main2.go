...
func orderedMoveOp(stacks []Stack, numCrates int, fromStack int, toStack int) []Stack {
	var temporaryStack Stack
	for i := 0; i < numCrates; i++ {
		newFrom, popped := Pop(stacks[fromStack])
		temporaryStack = Push(temporaryStack, popped)
		stacks[fromStack] = newFrom
	}
	for i := 0; i < numCrates; i++ {
		newTmp, popped := Pop(temporaryStack)
		newTo := Push(stacks[toStack], popped)
		stacks[toStack] = newTo
		temporaryStack = newTmp
	}
	return stacks
}
...
func main() {
	...
	for fileScanner.Scan() {
		...
		if parseMode == crate {
			...
			fromIx--
			toIx--
			stacks = orderedMoveOp(stacks, num, fromIx, toIx)
		}
	}
