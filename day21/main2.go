...
func main() {
	...
	for fileScanner.Scan() {
		...
		var elemInt int64
		elemInt, _ = strconv.ParseInt(line, 10, 64)
		// decryption KEY!
		elemInt *= 811589153
		elem := Elem{
			value: elemInt,
	...
	for mixRound := 0; mixRound < 10; mixRound++ {
		nextIterElem := iterDdList.head
		for nextIterElem != nil {
			elem := nextIterElem.elemP
			if elem.value > 0 {
				movePos := elem.value % int64(workDdList.len-1)
				for i := 0; i < int(movePos); i++ {
					workDdList.moveRight(elem)
				}
			} else if elem.value < 0 {
				valFloat := int64(math.Abs(float64(elem.value)))
				movePos := valFloat % int64(workDdList.len-1)
				for i := 0; i < int(movePos); i++ {
		...
		fmt.Printf("After %v round of mixing:\n", mixRound+1)
		//workDdList.print()
	}
