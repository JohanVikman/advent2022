...
// func dirSize(t *Tree) {
// 	if t.size > 0 {
// 		dirSizes = append(dirSizes, t.size)
// 	}
// }
...
func main() {
	...
	neededSpace := 30000000 - (70000000 - fileTree.size)
	fmt.Printf("Need %v\n", neededSpace)
	if err := f.Close(); err != nil {
		log.Fatal(err)
	}
	var dirSizes []int
	fileTree.Walk(func(t *Tree) {
		if t.size >= neededSpace {
			dirSizes = append(dirSizes, int(t.size))
		}
	})
